		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			elseif ( $aColumns[$i] == 'tindakan' )
            {
				...
				if($tindakan == 'cc1') {																										
					...
					$row[] .= ''."<a href='#' onclick=\"window.open('window_flat.php?option=MailTL&filetopen=MailTLKendaliMasuk&NId=$_GET[id]&GIR_Id=$GIR_Id','Cetak Kendali Masuk', 'width=575px, height=500px, center=yes,status=no, scroll=no, resizable=yes, left=350px, top=100px');\"><center><img src=images/print.png title='Cetak Kendali Masuk'></center></a>".' ';
				} else {
					$row[] .= ''."".'';				 
				}
			}			
