...
session_start();
	...
	$bColumns = array('GIR_Id', 'waktu', 'RoleId_From', 'RoleId_To', 'ReceiverAs', 'Msg', 'tindakan', 'RoleId_From2');
...
//	$aColumns = array( 'NId', 'NTipe', 'StatusReceive', 'Nomor', 'Hal', 'Tgl', 'InstansiPengirim' );
	...
	//Testing Ambil Table v_jejak_masuk
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
            ...
            {
				$sql3 = mysql_query("select * from v_jejak_surat where NId = '$NId'");
				$res3 = mysql_fetch_array($sql3);
				$panggilgir = $res3[GIR_Id];
				...
				{  
					...
					."<a href='#' onclick=\"window.open('window_flat.php?option=MailTL&filetopen=MailTLKendaliKeluar&NId=$NId&GIR_Id=$panggilgir','Cetak Kendali Keluar', 'width=575px, height=500px, center=yes,status=no, scroll=no, resizable=yes, left=350px, top=100px');\"><center><img src=images/print.png title='Cetak Kendali Keluar'></center></a>".' ';
				}  
				else 
				{
					$row[] .= ''.' '."<a href='#' onclick=\"window.open('window_flat.php?option=MailTL&filetopen=MailTLKendaliKeluar&NId=$NId&GIR_Id=$panggilgir','Cetak Kendali Keluar', 'width=575px, height=500px, center=yes,status=no, scroll=no, resizable=yes, left=350px, top=100px');\"><center><img src=images/print.png title='Cetak Kendali Keluar'></center></a>".' ';
				} 
			}			
			else if ( $aColumns[$i] != ' ' )
			{
