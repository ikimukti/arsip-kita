	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
            ...
            elseif ( $aColumns[$i] == 'zz' )
            {
				...
				if ($hasil222[NId] == "")  
				{  
					$row[] .= ''."<a href=handle.php?option=LogSuratMasuk&task=delete&ids=$NId onclick=\"return confirm('Apakah Surat Benar-Benar Akan Dihapus??')\"><center><img src=images/delete.png title='Hapus Surat'></center></a>".' '."<a href='#' onclick=\"window.open('window_flat.php?option=MailTL&filetopen=MailTLKendaliMasuk&NId=$NId&GIR_Id=$panggilgir','Cetak Kendali Masuk', 'width=575px, height=500px, center=yes,status=no, scroll=no, resizable=yes, left=350px, top=100px');\"><center><img src=images/print.png title='Cetak Kendali Masuk'></center></a>".' ';
				}  
				else 
				{
					//$row[] .= ''.' ';
