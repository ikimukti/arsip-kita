		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			elseif ( $aColumns[$i] == 'RoleId_To' )
        	{
				...
				else
				{
					...
					$sql2 = "select * from inbox_receiver where NId='".$_REQUEST["id"]."'";
							$res2 = mysql_query($sql2);
							$dataq = mysql_fetch_array($res2);
							$roleid = $dataq["To_Id"];
							$rec = $dataq["ReceiverAs"];
					...
					else
					{
						if ($rec == 'to') {
							...
							$row[] .= ''.$roleid."<br> <b><i>Tembusan :</i></b> $hasil2221[tembusan]".'';
						}
					}
				}
        	}
			...
			elseif ( $aColumns[$i] == 'tindakan' )
            {
				...
				elseif($tindakan == 'to') {																										
					$row[] .= ''."<a href='#' onclick=\"window.open('window_flat.php?option=MailTL&filetopen=MailTLKendaliMasuk&NId=$_GET[id]','Cetak Kendali Masuk', 'width=575px, height=500px, center=yes,status=no, scroll=no, resizable=yes, left=350px, top=100px');\"><center><img src=images/print.png title='Cetak Kendali Masuk'></center></a>".' ';
				}elseif($tindakan == 'to_keluar') {																										
					$row[] .= ''."<a href='#' onclick=\"window.open('window_flat.php?option=MailTL&filetopen=MailTLKendaliKeluar&NId=$_GET[id]','Cetak Kendali Keluar', 'width=575px, height=500px, center=yes,status=no, scroll=no, resizable=yes, left=350px, top=100px');\"><center><img src=images/print.png title='Cetak Kendali Keluar'></center></a>".' ';
				}elseif($tindakan == 'to_tl') {																										
					$row[] .= ''."<a href='#' onclick=\"window.open('window_flat.php?option=MailTL&filetopen=MailTLKendaliMasuk&NId=$_GET[id]','Cetak Kendali Masuk', 'width=575px, height=500px, center=yes,status=no, scroll=no, resizable=yes, left=350px, top=100px');\"><center><img src=images/print.png title='Cetak Kendali Masuk'></center></a>".' ';
				}
				else {
					$row[] .= ''."".'';				 
